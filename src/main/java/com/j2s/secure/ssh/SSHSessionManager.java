...
enum SSHSessionManager {
	...
	public SSHSession getSession(String sessionKey)
			throws SSHSessionNotFoundException, SSHSessionNotConnectionException {
	...
	private void _close(String sessionKey) {
		SSHSession sshSession = _removeSession(sessionKey);
		_close(sshSession);
	}

	private void _close(SSHSession sshSession) {
		try {
			sshSession.close();
		} catch (IOException e) {
			// nothing...
